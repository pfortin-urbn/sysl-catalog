...
var (
	...
	outputType        = kingpin.Flag("type", "Type of output").HintOptions("html", "markdown").Short('t').Default("markdown").String()
	...
	server            = kingpin.Flag("serve", "Start a http server and preview documentation").Short('s').Bool()
	...
	enableRedoc       = kingpin.Flag("redoc", "Generate redoc for specs imported from openapi. Must be run on a git repo.").Short('r').Default("false").Bool()
)
